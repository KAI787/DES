...
static long32 f(ulong32 r, unsigned char subkey[8])//  r:ringt block   subkey：密钥
{
	...
	for (int i = 0; i <= 7; i++)     //根据plaintext_32bit_expanded_to_48bit_table这张表, 把s中的
		for (int j = 0; j <= 5; j++) //4字节共32位扩展成48位并保存到数组plaintext中
		{
			if (j == 0)
				plaintext[i] = 0;
			if (j == 0)
			{
				...
				{
					plaintext[i] += ((s[(i - 1) / 2] & bytebit[3 + 4 * ((i + 1) % 2)]) == bytebit[3 + 4 * ((i + 1) % 2)]);
				}
			}
			else if (j >= 1 && j <= 4)
			{
				plaintext[i] += ((s[i / 2] & bytebit[j - 1 + 4 * (i % 2)]) == bytebit[j - 1 + 4 * (i % 2)]);
			}
			if (j != 5)
			...
			{
				if (i == 7)
					plaintext[i] += ((s[0] & bytebit[0]) == bytebit[0]);
				else
				{
					plaintext[i] += ((s[(i + 1) / 2] & bytebit[4 * ((i + 1) % 2)]) == bytebit[4 * ((i + 1) % 2)]);
				}
			}
		}
	for (int i = 0; i <= 7; i++)	//把plaintext中的8个字节与subkey中8个字节异或
	{								//异或后的值保存到plaintext内
		plaintext[i] = plaintext[i] ^ subkey[i];
	}
	for (int i = 0; i <= 7; i++)			//取出plaintext[i]查表sbox[i]得4位数, 循环8次可得8个4位数
	{								//按从左到右顺序合并得32位数保存到数组m中
		int p = 0, r = 0, l = 0;
		r = ((((plaintext[i] & bytebit[2]) >> 5) << 1) + (plaintext[i] & 1));
		l = (plaintext[i] >> 1) & 15;
		p = r * 16 + l;
		if ((i % 2) == 0)
			m[i / 2] = 0;
	...
	for (int i = 0; i <= 31; i++)			//根据sbox_perm_table把m中的32位数打乱保存到数组t中
	{
		if (i % 8 == 0)
			t[i / 8] = 0;
		t[i / 8] += ((m[(sbox_perm_table[i] - 1) / 8] & bytebit[(sbox_perm_table[i] - 1) % 8]) == bytebit[(sbox_perm_table[i] - 1) % 8]);
		if (((i + 1) % 8) != 0)
			t[i / 8] = t[i / 8] << 1;
